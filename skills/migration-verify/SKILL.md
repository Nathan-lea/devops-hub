...
description: >
  ...
  Web server migration). Use after running scripts/migration/migration_data_verify.sh
  to confirm source and target are in sync before traffic cutover. Covers row
  count comparison, CRC32 checksum, sequence validation, file-level rsync
  dry-run, and generates a go/no-go report. Reuses scripts/migration/ workflow.
tools_allowed:
  - bash
  - mysql
  - psql
...
本 skill 复用并增强 `scripts/migration/migration_data_verify.sh` 的工作流。
...
ssh -o BatchMode=yes -o ConnectTimeout=5 ops@SOURCE_IP echo "SSH ok" 2>/dev/null || echo "需配置 SSH 免密"
...
# 方式二：通过 Ansible 在管控节点执行
ansible-playbook playbooks/03-daily-inspection.yml \
  -e "migration_verify=true source=${SOURCE_IP} target=${TARGET_IP} type=mysql"
```
...
DIFF=$(rsync -an --stats "ops@$SOURCE_IP:$FILE_PATH/" "ops@$TARGET_IP:$FILE_PATH/" 2>/dev/null)
...
结论:
- 行数校验: $([ "$ROW_MISMATCH" -eq 0 ] && echo "✅ 全部一致" || echo "❌ 存在不一致")
- CRC32 校验: $([ "$CRC_MISMATCH" -eq 0 ] && echo "✅ 全部一致" || echo "❌ 存在不一致")
- 序列校验: $([ "$SEQ_MISMATCH" -eq 0 ] && echo "✅ 全部一致" || echo "⚠️ 需修正")
- 文件校验: $([ "$FILE_DIFF" -le 2 ] && echo "✅ 无差异" || echo "⚠️ 存在差异")

最终判定: $([ "$ROW_MISMATCH" -eq 0 ] && [ "$CRC_MISMATCH" -eq 0 ] && echo "✅ GO - 可以切换流量" || echo "❌ NO-GO - 禁止切换流量")
REPORT_EOF
...
- 如果校验发现不一致：
  ...
  3. 等待同步追平后重新校验
  4. 如果同步无法追平，按 `docs/migration/06-迁移通用保障与检查清单.md` 的回滚预案执行
- 如果已执行流量切换后发现数据不一致：
  ```bash
  # 按 docs/migration/ 中的回滚 SOP 执行
  bash scripts/migration/migration_rollback.sh --type dns --action rollback
  ```
...
| SSH 连接失败 | 免密未配置 | `ssh-copy-id ops@TARGET_IP` |
...
| 产物 | 关系 |
|------|------|
| `scripts/migration/migration_data_verify.sh` | 被复用的核心校验脚本 |
| `scripts/migration/pre_migration_check.sh` | 迁移前环境检查（校验前置） |
| `scripts/migration/migration_rollback.sh` | 校验失败后的回滚脚本 |
| `docs/migration/06-迁移通用保障与检查清单.md` | 校验方法论与回滚预案 |
| `docs/migration/01-MySQL迁移至TiDB-SOP.md` | MySQL 迁移校验步骤 |
| `docs/migration/02-PostgreSQL迁移至TiDB-SOP.md` | PG 迁移校验步骤 |
| `docs/migration/05-Web服务器迁移-SOP.md` | Web 迁移文件校验步骤 |
