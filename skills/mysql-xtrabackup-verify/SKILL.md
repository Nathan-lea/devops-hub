...
description: >
  ...
  running scripts/mysql_full_backup.sh to confirm backup is restorable, or
  during disaster recovery drills. Checks xtrabackup_info existence, performs
...
本 skill 复用并增强 `scripts/mysql_full_backup.sh` 的校验逻辑。
...
- 如果验证发现备份不可用，**立即重新执行备份**：
  ```bash
  bash scripts/mysql_full_backup.sh
  ```
...
| BACKUP_PASS 未设置 | 环境变量缺失 | `export BACKUP_PASS=$(ansible-vault view ...)` |

## 与本仓库的关系

| 产物 | 关系 |
|------|------|
| `scripts/mysql_full_backup.sh` | 被验证的备份脚本 |
| `docs/07-备份与容灾方案.md` | 3-2-1 备份策略设计 |
| `docs/04-数据库运维方案.md` | MySQL 运维文档 |
| `configs/rules/` | MySQL 备份告警规则 |
| `playbooks/02-mysql-maintenance.yml` | MySQL 维护 Playbook |
