...
description: >
  ...
  and physical (pg_basebackup) backups. Use after running
  scripts/postgresql_backup.sh to confirm backups are restorable, or during
  disaster recovery drills. Checks backup file completeness, test restore to
  a temporary instance, row count and sequence validation, and S3 remote copy
...
本 skill 复用并增强 `scripts/postgresql_backup.sh` 的校验逻辑，支持逻辑备份和物理备份两种模式。
...
- 如果验证发现备份不可用，**立即重新执行备份**：
  ...
  # 或通过 Ansible
  ansible-playbook playbooks/02-postgresql-maintenance.yml -l customer_globex_pg
  ```
...
| PG_PASS 未设置 | 环境变量缺失 | `export PG_PASS=$(ansible-vault view ...)` |

## 与本仓库的关系

| 产物 | 关系 |
|------|------|
| `scripts/postgresql_backup.sh` | 被验证的备份脚本 |
| `docs/07-备份与容灾方案.md` | 3-2-1 备份策略设计 |
| `docs/04-数据库运维方案.md` | PostgreSQL 运维文档 |
| `configs/rules/postgresql.yml` | PostgreSQL 备份告警规则 |
| `playbooks/02-postgresql-maintenance.yml` | PostgreSQL 维护 Playbook |
